...
from FYP.logistic_sgd import load_data
...
class Model(object):
    def __init__(self, input, n_in, n_hidden_layer, n_hidden_node, n_circuit, n_out,
        ...
        for index in range(n_circuit-1):
        ...
        self.circuits.append(
            Circuit(
                input=input,
                n_hidden_layer=n_hidden_layer,
                n_hidden_node=[n - (n_circuit-1)*(n // n_circuit) for n in n_hidden_node],
                n_in=n_in,  # sharing same input
                train=train,
                drop_type=drop_type,
                drop=mask[n_circuit],
...
def evaluate(dataset,
    ...
    for circuit in range(n_circuit):
    ...
    while epoch < n_epochs:
        ...
        index = [i for i in range(n_train_batches)]
        shuffle(index)
        ...
        valid_losses = [validate_model(i) for i in range(n_valid_batches)]
    ...
    #       .format(best_validation_loss * 100., this_test_loss * 100.))
    # print(('The code for file ' +
    #        os.path.split(__file__)[1] +
    #        ' ran for %.2fm' % ((end_time - start_time) / 60.)), file=sys.stderr)
    # Weight distribution
    weight_matrix = []
    for layer_index in range(len(classifier.params)):
